...
# filter for one year
  just.yr <- sys.traits %>%
    filter(Year == yr)
  # shuffle site status for jus† one year
  ...
  shuffled.means <- just.yr %>%
    ...
    summarise(AVGrandomSpec = mean(avg_specialization), AVGrandomBody = mean(avg_bodysize))
...
ggplot(randomized.comms, aes(x=AVGrandomBody, fill=ShuffledSiteStatus)) +
  geom_histogram(bins = 12) + 
    ...
    mapping =aes(xintercept =avg_bodysize, linetype = SiteStatus)) + labs(x="Avergae Body Size shuffled")
...
sum(randomized.comms$AVGrandomBody[
  randomized.comms$ShuffledSiteStatus == "control"] <= obs.means$avg_bodysize[obs.means$SiteStatus == "control"])/
...
```{r, eval=FALSE}
sum(randomized.comms$AVGrandomBody[
  randomized.comms$ShuffledSiteStatus == "mature"] >= obs.means$avg_bodysize[obs.means$SiteStatus == "mature"])/
...
ggplot(randomized.comms, aes(x=AVGrandomSpec, fill=ShuffledSiteStatus)) +
  geom_histogram(bins = 12) + 
    geom_vline(obs.means,
    mapping =aes(xintercept =avg_specialization, linetype = SiteStatus)) + labs(x="Avergae Specialization shuffled")
...
We can reject our null hypothesis because the p value is very low (almost 0), and therefore there is little probability that we would get our observed statistic if the null hypothesis (the traits in mature and control communities the same) was true. 
