...
head(rownames(sp_occ))
head(rownames(sp_mass))
head(colnames(sp_mass))
```
...
```{r}
# your code here
eat_network <- matrix(0, nrow = length(row_labs_sp), ncol = length(row_labs_sp))
rownames(eat_network) <- row_labs_sp
colnames(eat_network) <- row_labs_sp
for (i in 1:nrow(eat_network)){
  for(j in 1:nrow(eat_network)){
    eat_network[i,j] <- probEat(sp_mass[i,1], sp_mass[j,1])
...
```{r}
# your code here
eat_network <- sweep(eat_network, 2, carnivores, `*` )
head(eat_network)
```
...
###  your code here
year_2 <- sp_occ[,2]
year_2_empty <- eat_network * year_2
year_2_empty <- sweep(year_2_empty, MARGIN = 2, year_2, `*`)
sum(rowSums(year_2_empty)>0)
```
...
```{r}
# Each item in the list is an adjacency matrix
allYear_network <- vector("list", ncol(sp_occ))
for (i in 1:ncol(sp_occ)){
  year_i <- sp_occ[,i]
  year_inew <- eat_network * year_i
  year_inew <- sweep(year_inew, MARGIN = 2, year_i, `*`)
  allYear_network[[i]] <- year_inew  #stores matrix in list
}
print(allYear_network[[5]][1:5,1:5])
```
...
igraph_network_list <- list()
for (i in 1:length(allYear_network)){
  igraph_network_list[[i]] <- graph_from_adjacency_matrix(allYear_network[[i]],mode = "directed", weighted =TRUE, diag = FALSE)
         }          
                                                      
igraph_network_list
str(igraph_network_list)
...
grouped_allYear <- carnivores[V(igraph_network_list[[22]])$name]


###  convert to a network 3d object
#convert to a network 3d object
food_web_net3d_22 <- igraph_to_networkD3(igraph_network_list[[22]], group = grouped_allYear, 
                                       what = "both")

###  plot the network
forceNetwork(Links = food_web_net3d_22$links,
             Nodes = food_web_net3d_22$nodes,
            Source = "source", Target = "target",
            Value = "value",  NodeID = "name",
             Group = "group", arrows = TRUE,
...
# your code here
###  assign groups as carnivore or herbivore
grouped_allYear <- carnivores[V(igraph_network_list[[2]])$name]


###  convert to a network 3d object
#convert to a network 3d object
food_web_net3d_2 <- igraph_to_networkD3(igraph_network_list[[2]], group = grouped_allYear, 
                                       what = "both")

###  plot the network
forceNetwork(Links = food_web_net3d_2$links,
             Nodes = food_web_net3d_2$nodes,
            Source = "source", Target = "target",
            Value = "value",  NodeID = "name",
             Group = "group", arrows = TRUE,
            opacity = 0.8, zoom=TRUE)
...
# your code here
###  assign groups as carnivore or herbivore
grouped_allYear <- carnivores[V(igraph_network_list[[15]])$name]


###  convert to a network 3d object
#convert to a network 3d object
food_web_net3d_15 <- igraph_to_networkD3(igraph_network_list[[15]], group = grouped_allYear, 
                                       what = "both")

###  plot the network
forceNetwork(Links = food_web_net3d_15$links,
             Nodes = food_web_net3d_15$nodes,
            Source = "source", Target = "target",
            Value = "value",  NodeID = "name",
             Group = "group", arrows = TRUE,
            opacity = 0.8, zoom=TRUE)
